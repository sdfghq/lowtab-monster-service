...
public sealed class GetArticlesHandlerTests : IDisposable, IAsyncDisposable
{
    ...
    public async Task GetArticlesHandler_Handle_Successful()
    {
        ...
        var request = new GetArticlesQuery { Offset = 0, Limit = 10 };
        ...
        result = await handler.Handle(new GetArticlesQuery { Offset = 0, Limit = 1 }, CancellationToken.None);
        result.Should().NotBeNull();
        result.TotalFound.Should().Be(result.TotalFound).And.Be(2);
        Assert.Equivalent(1, result.Items.Count);

        result = await handler.Handle(new GetArticlesQuery { Offset = 1, Limit = 1 }, CancellationToken.None);
        result.Should().NotBeNull();
        result.TotalFound.Should().Be(result.TotalFound).And.Be(2);
        Assert.Equivalent(1, result.Items.Count);
    }
